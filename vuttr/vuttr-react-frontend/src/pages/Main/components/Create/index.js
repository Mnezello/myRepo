    ...
    <Modal {...rest} display={open ? 1 : 0}>
      <Container>
        ...
        <Body context={{form}} schema={schema} onSubmit={handleSubmit}>
          <Input placeholder="Enter tool name" label="Tool title" value="" name="title" onChange={onChange} />
          <Input 
          ...
          />
          <Input
            placeholder="Tags with no hash and space"
            label="Tags"
            value=""
            name="tags"
            onChange={onChange}
        ...
        <Footer>
          ...
          <Button icon={<MdCloudUpload />} border type="submit" text="Add tool" onClick={() => handleSubmit()} />
        </Footer>
      </Container>
    </Modal>
  );
